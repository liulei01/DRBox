...
// Usage:
//   convert_annoset_r [FLAGS] ROOTFOLDER/ LISTFILE DB_NAME
//
...
int main(int argc, char** argv) {
  ...
  AnnotatedDatumR_AnnotationType type;
  ...
  if (anno_type == "classification") {
    ...
    type = AnnotatedDatumR_AnnotationType_RBOX;
    LabelMap label_map;
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    if (anno_type == "classification") {
      ...
      status = ReadRichImageToAnnotatedDatumR(filename, labelname, resize_height,
          resize_width, min_dim, max_dim, is_color, enc, type, label_type,
          name_to_label, &anno_datum);
      anno_datum.set_type(AnnotatedDatumR_AnnotationType_RBOX);
    }
    if (status == false) {
