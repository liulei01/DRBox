...
def CreateAnnotatedRDataLayer(source, batch_size=32, backend=P.Data.LMDB,
        output_label=True, train=True, label_map_file='', anno_type=None,
        transform_param={}, batch_sampler=[{}], name="data"):
    if train:
        kwargs = {
    ...
    if output_label:
        ntop = 2
    annotated_r_data_param = {
        'label_map_file': label_map_file,
        'batch_sampler': batch_sampler,
        }
    if anno_type is not None:
        annotated_r_data_param.update({'anno_type': anno_type})
    return L.AnnotatedRData(name=name, annotated_r_data_param=annotated_r_data_param,
...
def VGGNetBodyCut(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
        dilated=False, nopool=False, dropout=True, freeze_layers=[], dilate_pool4=False):
    kwargs = {
            'param': [dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
            'weight_filler': dict(type='xavier'),
...
# Defined by LL:
#  prior_widths: width and height for the boxes, which is fixed during training and testing 
#  min_sizes and max_sizes are not used
#  rotate_angles: the angles for rboxes

def CreateMultiRBoxHead(net, data_layer="data", num_classes=[], from_layers=[],
        rotate_angles=[], prior_widths=[], prior_heights=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, lr_mult=1,
        use_scale=True, prior_variance = [0.1],
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0,
        conf_postfix='', loc_postfix='', regress_size = False, regress_angle = False, **bn_param):
    assert num_classes, "must provide num_classes"
    assert num_classes > 0, "num_classes must be positive number"
    if normalizations:
        assert len(from_layers) == len(normalizations), "from_layers and normalizations should have same length"
    ...
    for i in range(0, num):
        ...
        # Estimate number of priors per location given provided parameters.
        num_priors_per_location = len(rotate_angles) * len(prior_widths)
        step = []
        if len(steps) > i:
            step = steps[i]

        # Create location prediction layer.
        name = "{}_mbox_loc_plane{}".format(from_layer, loc_postfix)
        num_loc_output = num_priors_per_location * num_param;
        if not share_location:
            num_loc_output *= num_classes
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_loc_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
        ...
        # Create confidence prediction layer.
        name = "{}_mbox_conf_plane{}".format(from_layer, conf_postfix)
        num_conf_output = num_priors_per_location * num_classes;
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_conf_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
        permute_name = "{}_perm".format(name)
        ...
        # Create prior generation layer.
        name = "{}_mbox_priorrbox_plane".format(from_layer)
        net[name] = L.PriorRBox(net[from_layer], net[data_layer], prior_widths = prior_widths, prior_heights = prior_heights,
                clip=clip, variance=prior_variance, offset=offset, rotated_angles=rotate_angles,
        ...
        if img_height != 0 and img_width != 0:
            if img_height == img_width:
                ...
                net.update(name, {'img_h': img_height, 'img_w': img_width})
        priorrbox_layers.append(net[name])

        # Create objectness prediction layer.
        if use_objectness:
            name = "{}_mbox_objectness_plane".format(from_layer)
            num_obj_output = num_priors_per_location * 2;
            ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
                num_output=num_obj_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
            permute_name = "{}_perm".format(name)
    ...
    net[name] = L.Concat(*loc_layers, axis=1)
    mbox_layers.append(net[name])
    name = "mbox_conf_plane"
    net[name] = L.Concat(*conf_layers, axis=1)
    mbox_layers.append(net[name])
    name = "mbox_priorbox_plane"
    net[name] = L.Concat(*priorrbox_layers, axis=2)
    mbox_layers.append(net[name])
    if use_objectness:
        name = "mbox_objectness_plane"
        net[name] = L.Concat(*objectness_layers, axis=1)
        mbox_layers.append(net[name])
